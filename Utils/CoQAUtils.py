...
class BatchGen:
    def __init__(self, opt, data, use_cuda, vocab, char_vocab, evaluation=False):
        ...
        if 'PREV_ANS' in self.opt:
            ...
            self.prev_ans = 0
            print("no pre ans appended to current question")

        if 'PREV_QUES' in self.opt:
            self.prev_ques = self.opt['PREV_QUES']
        else:
            self.prev_ques = 0
            print("no pre que appended to current question")
    ...
    def __iter__(self):
        ...
        for datum in data:
            if not self.evaluation:
                ...
                datum['qas'] = [qa for qa in datum['qas'] if len(qa['annotated_answer']['word']) == 1 or
                                qa['answer_span'][1] - qa['answer_span'][0] < MAX_ANS_SPAN]
                datum['qas'] = [qa for qa in datum['qas'] if len(qa['annotated_question']['wordid']) < self.ques_max_len]
            ...
            for i in range(qa_len):
                ...
                if 'BERT' in self.opt:
                    ...
                    #     query_bert_offsets[i, :len(now_bert_offsets), :] = torch.tensor(now_bert_offsets, dtype = torch.long)
                    # except:
                    #     print(i,j, datum['qas'][j])
                    #     print(query_bert_offsets.size(), ques_words, now_bert, now_bert_offsets)
            ...
            # 修改将所有.cuda(async=True)中的async=True去掉
            if self.use_char_cnn:
