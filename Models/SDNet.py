...
from Models.Layers import MaxPooling, CNN, dropout, RNN_from_opt, set_dropout_prob, weighted_avg, set_seq_dropout, Attention, DeepAttention, DeepAttention_2, LinearSelfAttn, GetFinalScores, Question_Selector, My_GRU
...
class SDNet(nn.Module):
    def __init__(self, opt, word_embedding):
        ...
        #questions selection
        self.selector = Question_Selector(opt, multi_layers = False)
        ...
        # self.deep_attn = DeepAttention_2(opt, abstr_list_cnt=opt['in_rnn_layers'],
        ...
        # self.high_lvl_cont_rnn, high_lvl_cont_rnn_output_size = RNN_from_opt(
        #     context_rnn_output_size * opt['in_rnn_layers'],
        #     opt['highlvl_hidden_size'], num_layers=opt['question_high_lvl_rnn_layers'], concat_rnn=True)

        self.after_deep_attn_size = 2*self.deep_attn_output_size + self.deep_attn_input_size + addtional_feat + vocab_dim
        self.self_attn_input_size = self.after_deep_attn_size
        self_attn_output_size = self.deep_attn_output_size

        #recurrent reasoning
        self.rGRU = My_GRU(self.deep_attn_output_size, opt['deep_att_hidden_size_per_abstr'])
    ...
    def forward(self, x, x_single_mask, x_char, x_char_mask, x_features, x_pos, x_ent, x_bert, x_bert_mask, x_bert_offsets, q, q_mask, q_char, q_char_mask, q_bert, q_bert_mask, q_bert_offsets, context_len):
        ...
        # question select
        ques_similarity = self.selector(ques_rnn_layers)
        x_rnn_layers = [
            torch.matmul(ques_similarity, x.reshape(x.size(0), -1)).reshape(x.size(0), x.size(1), x.size(2)) for x in
            x_rnn_layers]
        ques_rnn_layers = [
            torch.matmul(ques_similarity, ques.reshape(ques.size(0), -1)).reshape(ques.size(0), ques.size(1), ques.size(2)) for ques in ques_rnn_layers]
        ...
        # # rnn with context
        # x_highlvl = self.high_lvl_cont_rnn(torch.cat(x_rnn_layers,2), x_mask)
        # x_rnn_layers.append(x_highlvl)
        ...
        x_gru_output = self.rGRU(x_rnn_after_inter_attn)


        # deep self attention
        if x_cemb is None:
            x_self_attn_input = torch.cat([x_gru_output, x_rnn_after_inter_attn, x_inter_attn, x_word_embed], 2)
        else:
            x_self_attn_input = torch.cat([x_gru_output, x_rnn_after_inter_attn, x_inter_attn, x_cemb, x_word_embed], 2)
