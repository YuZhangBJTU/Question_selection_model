...
class StackedBRNN(nn.Module):
    ...
    def forward(self, x, x_mask, return_list=False, x_additional = None):
        ...
        if return_list:
            #返回最后一个layer的output = batch * seq_len * hidden_size，以及将所有ayer的output放到一个列表返回
...
class Attention(nn.Module):
    ...
    def forward(self, x1, x2, x2_mask, x3 = None, drop_diagonal=False):
        ...
        # scores: batch x word_num1 x word_num2
        #.eq(0)返回张量中与0比较后的布尔值，是0为True
        empty_mask = x2_mask.eq(0).unsqueeze(1).expand_as(scores)
        #.data相当于做了内存不共享的拷贝，.masked_fill用-float('inf')填充mask中的True值，-float('inf')为负无穷
...
# History-of-Word Multi-layer inter-attention
class DeepAttention_2(nn.Module):
    def __init__(self, opt, abstr_list_cnt, deep_att_hidden_size_per_abstr, correlation_func=1, word_hidden_size=None):
        super(DeepAttention_2, self).__init__()
        ...
        self.rnn_input_size = rnn_input_size
        self.rnn, self.output_size = RNN_from_opt(rnn_input_size, opt['highlvl_hidden_size'], num_layers=1)
        self.Attention = Attention(att_size, deep_att_hidden_size_per_abstr, correlation_func=correlation_func)
    ...
    def forward(self, x1_word, x1_abstr, x2_word, x2_abstr, x1_mask, x2_mask, return_bef_rnn=False):
        ...
        x1_att = torch.cat(x1_word + x1_abstr, 2)
        x2_att = torch.cat(x2_word + x2_abstr, 2)

        x1 = torch.cat(x1_abstr, 2)

        for i in range(3):
            atten_self_c = self.Attention(x1_att, x1_att, x1_mask)
            atten_self_q = self.Attention(x2_att, x2_att, x2_mask)
            atten_c = self.Attention(atten_self_c, atten_self_q, x2_mask)
            atten_q = self.Attention(atten_self_q, atten_self_c, x1_mask)
            x1_att = atten_c
            x2_att = atten_q
            x1 = torch.cat(x1_att, 2)
...
class attentive_model(nn.Module):
    def __init__(self, input_size, hidden_size, correlation_func, att_size,deep_att_hidden_size_per_abstr,
                 num_layers, rnn_type = nn.LSTM, concat_layers = False, bidirectional = True, add_feat=0):
        super(attentive_model, self).__init__()
        self.Attention = Attention(att_size, deep_att_hidden_size_per_abstr, correlation_func=correlation_func)

    def forward(self, x1, x2):
        for i in range(2):
            x1_attend = self.Attention(x1, x2, x1)
            x2_attend = self.Attention(x1, x2, x2)
...
class Question_Selector(nn.Module):
    def __init__(self, opt, multi_layers = False):
        super(Question_Selector, self).__init__()
        self.opt = opt
        self.multi_layers = multi_layers
        self.sigmoid = torch.nn.Sigmoid()

    #ques_rnn_output：batch(turns) * q_leng * ques_rnn_output_size
    #每个问题用一个向量表示，进行句子相似度计算
    def sentence_level_cosine_similarity(self, ques_rnn_output):
        question_representation = ques_rnn_output[:, -1, :].squeeze(1)  # batch * ques_rnn_output_size
        num = torch.mm(question_representation, question_representation.t())  # batch * batch
        denom = torch.mm(torch.norm(question_representation, dim=1).unsqueeze(1),
                         torch.norm(question_representation, dim=1).unsqueeze(0))
        cosine_sim = num / denom

        return cosine_sim

    def word_level_sim(self, ques_rnn_output):
        ques_1 = ques_rnn_output.unsqueeze(1)  # ques_1: b * 1 * ques_len * ques_rnn_output_size
        ques_2 = ques_rnn_output.unsqueeze(0).transpose(-2, -1)  # ques_2: 1 * b * ques_rnn_output_size * ques_len
        word_level_cosine_sim = torch.matmul(ques_1, ques_2)  # word_level_cosine_sim: b * b * ques_len * ques_len
        a = torch.norm(ques_1, dim=3)
        b = torch.norm(ques_2, dim=2)
        denom = torch.matmul(a.unsqueeze(-1), b.unsqueeze(-2))
        # denom = torch.mm(torch.norm(ques_1, dim=3).unsqueeze(-1), torch.norm(ques_2, dim=2).unsqueeze(-2))
        word_level_cosine_sim = word_level_cosine_sim / denom
        # 取单词相似度矩阵的平均池化值作为两个问句的相似性. out1: b * b
        out1 = nn.AvgPool2d(kernel_size=(word_level_cosine_sim.size(2), word_level_cosine_sim.size(3)))(
            word_level_cosine_sim).squeeze()
        # 取单词相似度矩阵的最大池化值作为两个问句的相似性
        out2 = nn.MaxPool2d(kernel_size=(word_level_cosine_sim.size(2), word_level_cosine_sim.size(3)))(
            word_level_cosine_sim).squeeze()
    ...
    def forward(self, ques_rnn_layers):
        if self.multi_layers:
            sentence_similaritys = []
            word_similaritys = []
            for i in ques_rnn_layers:
                sent_similarity = self.sentence_level_cosine_similarity(i)
                w_similarity, _ = self.word_level_sim(i)
                sentence_similaritys.append(sent_similarity)
                word_similaritys.append(w_similarity)
            sentence_similarity = sum(sentence_similaritys)
            word_similarity = sum(word_similaritys)

        else:
            ques = ques_rnn_layers[-1]
            sentence_similarity = self.sentence_level_cosine_similarity(ques)
            word_similarity, _ = self.word_level_sim(ques)
            sentence_similarity = self.sigmoid(sentence_similarity)


        ques_similarity = sentence_similarity * word_similarity
        ques_similarity = self.sigmoid(ques_similarity)
        ques_similarity = torch.triu(ques_similarity, diagonal=1) # batch * batch

        return ques_similarity
...
class My_GRU(nn.Module):
    ...
    def forward(self, x):
        ...
        h = torch.zeros(seq_len, self.hidden_size).to(x.device)
        output = torch.zeros(batch_size, seq_len, self.hidden_size).to(x.device)
